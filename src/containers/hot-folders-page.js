...
class HotFoldersPage extends React.Component {
  ...
  getPrintCodes(pageindex, pagesize) {
    api
      .getPrintCodesPaged(pageindex, pagesize)
      .then(response => {
        ...
        var add_page =
          response.data.count -
            parseInt(response.data.count / this.state.pagesize) >
          0
            ? 1
            : 0;
        paging.total =
          parseInt(response.data.count / this.state.pagesize) + add_page;
    ...
    if (!this.state.single_view) {
      display = (
        <div>
          <h2>Hot folders</h2>
          <HotFoldersTable
            printcodes={this.state.printcodes}
            editClick={printcode => {
              this.editPrintCode(printcode);
            }}
          />
          <Pager
      ...
      display = (
        <div>
          <HotFoldersView
            printcode={this.state.single_printcode}
            saveClick={printcode => {
              this.savePrintCode(printcode);
            }}
            cancelClick={() => {
              this.cancelPrintCode();
            }}
          />
        </div>
      );
